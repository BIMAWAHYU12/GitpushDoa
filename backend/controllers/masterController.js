...
const deleteRak = async (req, res) => {
  ...
  try {
    // 1. Cek apakah rak dipakai di barang
    const [cek] = await db.query(
      "SELECT COUNT(*) AS total FROM barang WHERE rak_id = ?",
      [id]
    );

    if (cek[0].total > 0) {
      return res.status(400).json({
        message: "Rak masih digunakan oleh barang, tidak bisa dihapus!",
      });
    }

    // 2. Hapus rak
    const [result] = await db.query(
      "DELETE FROM rak WHERE id_rak = ?",
      [id]
    );
    ...
    // fallback kalau masih ada FK lain yang lolos
    if (err.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(400).json({
        message: "Rak sedang digunakan dan tidak bisa dihapus!",
      });
    }
...
const createSupplier = async (req, res) => {
  ...
  try {
    await db.query("INSERT INTO supplier (nama_supplier) VALUES (?)", [
      nama_supplier,
    ]);
...
const updateSupplier = async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "UPDATE supplier SET nama_supplier = ? WHERE id_supplier = ?",
      [nama_supplier, id]
    );
...
const deleteSupplier = async (req, res) => {
  ...
  try {
    ...
    if (err.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(400).json({
        message: "Supplier sedang digunakan dan tidak bisa dihapus!",
      });
    }
    res.status(500).json({ error: err.message });
  }
...
const createOutlet = async (req, res) => {
  ...
  try {
    await db.query("INSERT INTO outlet (nama_outlet) VALUES (?)", [
      nama_outlet,
    ]);
...
const updateOutlet = async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "UPDATE outlet SET nama_outlet = ? WHERE id_outlet = ?",
      [nama_outlet, id]
    );
...
const deleteOutlet = async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "DELETE FROM outlet WHERE id_outlet = ?",
      [id]
    );
    ...
    if (err.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(400).json({
        message: "Outlet sedang digunakan dan tidak bisa dihapus!",
      });
    }
    res.status(500).json({ error: err.message });
  }
